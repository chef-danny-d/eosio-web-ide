...
class [[eosio::contract("hodl")]] hodl : public eosio::contract {
  private:
    uint32_t the_party = current_time_point().sec_since_epoch();
    ...
    int offset(uint32_t the_party) {
        print("Offset time is: ", the_party + 120, "\n");
        uint32_t off = the_party + 120;
        return off;
    }
  ...
  public:
    ...
    void deposit(name hodler, name to, eosio::asset quantity, std::string memo) {
      ...
      check(now() < offset(the_party), "Too late");
